...
class RRTPlanner:
    ...
    def compute_path_planning(self):
        """
        Executa o RRT com obstáculos circulares, retorna a lista de posições do caminho se encontrar, ou None.
        """
        self.tree = [self.start]

        for i in range(self.max_iter):
            rand_pt = sample_random_point(self.width, self.height)
            # pula amostras dentro de obstáculos
            if collides(rand_pt, self.obstacles):
                continue

            nearest_node = nearest(self.tree, rand_pt)
            new_node = steer(nearest_node, rand_pt, self.step_size)

            # verifica colisão do novo nó
            if collides(new_node.position, self.obstacles):
                continue
            self.tree.append(new_node)

            # Checa se chegou próximo ao objetivo e sem colisão
            if distance(new_node.position, self.goal.position) < self.step_size:
                if not collides(self.goal.position, self.obstacles):
